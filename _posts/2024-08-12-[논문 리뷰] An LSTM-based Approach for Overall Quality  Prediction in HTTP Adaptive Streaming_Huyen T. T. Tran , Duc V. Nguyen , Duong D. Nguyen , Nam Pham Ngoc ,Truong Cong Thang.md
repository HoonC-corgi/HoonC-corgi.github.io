...
본 섹션에서는 우선 제안하는 접근 방식의 예측 성능 평가를 위한 실험 환경을 명세한다.  
다음으로, 제안된 접근 방식에서 각 segment feature들의 역할에 대하여 이야기한다.  
마지막으로 기존의 네 가지 접근 방식과 성능을 비교한다.  
...
세션 동안 세그먼트의 버전이 고정된 상태로 품질 변화가 없고, stalling event가 없는 5가지 패턴,  
10초 주기의 주기적인 품질 변화가 있고, stalling event가 없는 10가지 패턴,  
0.25, 0.5, 1, 2, 3, 4초의 지속 시간을 가진 1회에서 6회의 stalling event를 포함하며 품질 변화가 없는 27가지 패턴,  
30개의 실제 스트리밍 세션은 [20], [21]의 두 가지 적응형 방법을 사용하여 모바일 네트워크의 대역폭 추적 자료 [22]를 이용한 스트리밍 테스트베드에서 실행하여 생성되었다.  
실제 스티리밍 세션은 품질 변화와 stalling event를 모두 포함하고 있다.  
...
제안된 접근 방식의 학습 과정에서는 **루트 평균 제곱 오차 (RMSE)** 손실 함수를 적용했다.  
...
제안된 접근 방식의 예측 성능을 평가하기 위해 **피어슨 상관 계수 (PCC)**와 **루트 평균 제곱 오차 (RMSE)** 두 가지 지표를 사용하며, 이 지표들은 100개의 테스트 세트에 대해 평균화된다.  
...
1) Dataset
   ...
   * 세션 생성 및 구성
     ...
     수작업 세션은 품질 변화와 stalling event가 없는 패턴, 주기적인 품질 변화만 있는 패턴, stalling event만 있는 패턴으로 나뉨.  
     실제 스트리밍 세션은 모바일 네트워크 대역폭 자료를 기반으로 스트리밍 테스트베드에서 생성되었으며, 품질 변화와 stalling event를 모두 포함하고 있음.  
   <br>
