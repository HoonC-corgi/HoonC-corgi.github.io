...
첫 째로, 상위 MAC(U-MAC)이 있는데, 이는 모든 인터페이스에 대한 MAC sub-layer의 공통 부분이다.  
...
첫 째, 비동기 tranmission mode는 MLD가 여러 링크에 비동기적으로 프레임을 전송할 수 있도록 한다. 
...
MPC 방식의 적용을 통해 노드가 경쟁에서 이기고 프레임을 전송할 수 있는 가능성이 높아지지만, SPC는 non-primary channel 인터페이스가 doze 항태로 유지되어 전력 소비를 줄일 수 있다. 
...
1. STA가 AP MLD로부터 2.4GHz 대역에서 AP MLD의 비콘 프레임을 수신함.(5GHz 대역에 대한 정보가 RNR 요소로 확인 가능)
...
1. STA MLD가 AP로 부터 비콘 프레임을 받고 안에 있는 TIM 요소에서 비트가 1로 설정된 것을 감지하면,
...
링크 상태를 실시간으로 평가, 트래픽을 분산하는 과정으로 이루어짐.
...
첫 째, non-STR STA는 다른 링크에서 전송을 수행하고 있기 때문에 사용 가능한 다른 링크에서의 DL, UL을 감지하지 못할 수 있다. 
